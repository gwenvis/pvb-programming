...
public class Vehicle : MonoBehaviour{
	...
	[SerializeField] [Range(50.0f, 80.0f)] private float jumpForce = 60f;
	[SerializeField] [Range(0.0f, 20.0f)] private float gravity = 10f;

	[SerializeField] private bool steerInAir = true;
	[SerializeField] private bool motorcycleTilt = false;
	...
	void Update(){
		...
		if(controllable){
			...
			if(Input.GetKey(KeyCode.S)){ speed = -acceleration; }
		...
		if(controllable && (nearGround || steerInAir)){
			...
			if(Input.GetKey(KeyCode.A)) { rotate = -steering; }
			if(Input.GetKey(KeyCode.D)){ rotate =  steering; }
